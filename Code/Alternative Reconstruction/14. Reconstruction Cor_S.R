...
## Read predictors Cor > 0.3
properties1 = read.csv("Output Data/PAGES 2k v.2.0.0/Properties/All.properties.select.csv")

## Select
properties2 = properties1 %>%
  dplyr::filter(abs(Max.Cor.CGR) > 0.3)
...
write.csv(predictors,"Output Data/Predictors/PAGS2k_Cor_S.csv",row.names = FALSE)
...
recon=function(parallel.ge){
  ...
  ## Predictors
  predictors=read.csv("Output Data/Predictors/PAGS2k_Cor_S.csv")
  ...
  Write.name=paste0("Z:/2022/Science9_SM/Output Data/Reconstructed CGR/All Ensemble_Cor_S/",parallel.ge,".csv")
...
for(parallel.ge in 1:nparas){
  ...
  ## read
  Read.name=paste0("Z:/2022/Science9_SM/Output Data/Reconstructed CGR/All Ensemble_Cor_S/",parallel.ge,".csv")
...
## Write
write.csv(Total.summary,"Output Data/Reconstructed CGR/SD/Total.summary.sd_Cor_S.csv",row.names = F)

# Comparison --------------------------------------------------------------

## Data
Total.summary1=read.csv("Output Data/Reconstructed CGR/SD/Total.summary.sd.csv")
Total.summary2=read.csv("Output Data/Reconstructed CGR/SD/Total.summary.sd_Cor_S.csv")
